...
export function memoryText(n){const names={helped:'helped',befriended:'befriended',threatened:'threatened',lied:'lied to',lieExposed:'lie exposed',stolen:'pickpocketed',theftDetected:'theft discovered',robbed:'robbed',robberyAttempt:'robbery attempted',attacked:'attacked',knockedOut:'knocked unconscious',killed:'killed',looted:'looted',betrayed:'betrayed',woken:'woken up',heardTobinRobbed:'heard about Tobin',heardTobinKilled:'heard Tobin was killed'};return Object.entries(names).filter(([key])=>n.memory[key]).map(([,label])=>label).join(', ')||'No shared history yet';}
...
export function learnRumours(s,id){if(id!=='vex')return;const n=s.npcs.vex,t=s.npcs.tobin;if(n.condition!=='conscious')return;if(t.condition==='dead'&&t.memory.killed){n.memory.heardTobinKilled=true;n.attitude='hostile';}else if(t.memory.robbed||t.memory.theftDetected){n.memory.heardTobinRobbed=true;if(n.attitude==='neutral'||n.attitude==='friendly')n.attitude='suspicious';}}
function talk(s,id,say){const n=s.npcs[id],m=n.memory;m.met=true;
 ...
 if(id==='vex'&&m.heardTobinKilled){say(s,'Vex: “The ceiling called Tobin’s death a retention event. He had a name. Keep your hands where I can see them.” Vex refuses trade and friendship.');return;}
 if(id==='vex'&&m.heardTobinRobbed){say(s,'Vex: “Tobin’s radio still works. He says you took his supplies. A rival I can respect. A mugger gets no training.” Vex is suspicious and will not befriend you.');return;}
...
export function actNPC(s,id,action,{say,award,startCombat}){const n=s.npcs[id],m=n.memory,d=NPCS[id];learnRumours(s,id);
 ...
 if(action==='Kill'&&n.condition==='unconscious'){betray(n);n.condition='dead';n.hp=0;m.killed=true;n.attitude='hostile';say(s,`${d.name} is dead. Their remaining possessions can be looted. ANNEX: “One fewer unresolved relationship.”`);return true;}
 ...
 if(action==='Help'){
  ...
  m.helped=true;if(n.attitude!=='hostile'&&!m.betrayed)n.attitude=id==='mara'?'friendly':'neutral';say(s,`You give ${d.name} ${cost}. ${n.attitude==='hostile'?'They accept the help, but do not forgive what happened.':'They remember the help. Talk, then Befriend to build an alliance.'}`);return true;
 }
 if(action==='Befriend'){
  if(n.attitude==='hostile'||m.betrayed||m.lieExposed||m.heardTobinRobbed){say(s,`${d.name} refuses. Help does not erase betrayal, violence, or Tobin’s warning.`);return true;}
 ...
 if(action==='Pickpocket'){
  ...
  if(!m.distracted&&n.attitude!=='friendly'){betray(n);m.theftAttempt=true;n.attitude='hostile';say(s,`${d.name} catches your hand. Distract them with a credible lie or gain their trust first. They are now hostile.`);return true;}
  const key=[d.pick,'potions','gold',...ITEM_KEYS,'key'].find(k=>n.inventory[k]>0);if(!key){say(s,`${d.name} has nothing to steal.`);return true;}
  const got=transfer(s,n,key,key==='gold'?2:1);betray(n);m.stolen=true;m.distracted=false;say(s,`You quietly take ${got} ${itemName[key]} from ${d.name}. They will discover the theft when you leave this room. It will not be forgotten.`);return true;
 }
 if(action==='Threaten'){
 ...
 if(action==='Rob openly'){
  ...
  m.robbed=true;say(s,`${d.name} surrenders ${takeAll(s,n)} rather than die. They remain alive, hostile, and remember the robbery. ${id==='tobin'?'Tobin reaches for a radio.':''}`);return true;
 }
