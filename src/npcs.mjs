...
export const NPCS = {
 ...
 ,skulker:{name:'Ratman skulker',archetype:'skulker',pack:'ratmen',role:'Thrown-object specialist',hp:10,damage:[1,2],thrown:[2,4],attitude:'neutral',stock:{gold:3,smokeBombs:1},equipped:[],refuse:['smokeBombs'],onHit:'bleeding',xp:35,home:1,routine:'guard',route:[1],abilities:{strength:8,dexterity:16,constitution:10,intelligence:11,wisdom:13,charisma:7},skills:['stealth','perception'],saves:['dexterity']}
 ,brute:{name:'Ratman brute',archetype:'brute',pack:'ratmen',role:'Slow and heavy',hp:24,damage:[5,7],attitude:'neutral',stock:{gold:4,potions:1},equipped:[],refuse:['potions'],onHit:'bleeding',xp:80,home:2,routine:'guard',route:[2],abilities:{strength:17,dexterity:9,constitution:16,intelligence:6,wisdom:9,charisma:5},skills:['athletics','intimidation'],saves:['strength','constitution']}
};
...
export function actNPC(s,id,action,{say,award,startCombat,witness=()=>[],deed=()=>{},bonus=()=>0,note=()=>'',standing=()=>{},rng=Math.random}){const n=s.npcs[id],m=n.memory,d=NPCS[id];
 ...
 if(action==='Pickpocket'){
  ...
  const got=transfer(s,n,key,key==='gold'?2:1);s.stolen[key]=id;deed('theft');standing(id,2,'you stole from one of them');award(s,'Five-Finger Discount');betray(n);m.stolen=true;m.distracted=false;say(s,`${line} You quietly take ${got} ${itemName[key]} from ${d.name}. They will discover the theft when you leave this room. It will not be forgotten.`);witness('theft',id);return true;
 }
 if(action==='Threaten'){
