...
let state=fresh(),target=null,busy=false,lastKey=0,actionOpen=false,toastTimer,bannerTimer,controls=null,feedback='',npcTab='social',camera={x:0,y:0,scale:1},visual={x:3,y:5},motion=null,frame=0;
...
function changed(){if(!state.dead&&!state.combat)save();render();endScreen();promptEvents();}
...
function action(label,confirmed=false){if(busy||anyDialog())return;if(label==='Kill'&&!confirmed){modal('Kill '+NPCS[target].name+'?',state.npcs[target].condition==='unconscious'?'They are unconscious and cannot fight back. This kills them permanently in this playthrough.':NPCS[target].name+' will fight back. This starts lethal combat.',[['Cancel',()=>{}],['Kill',()=>action(label,true)]]);return;}stopHold();busy=true;const wasCombat=!!state.combat,oldSerial=state.logSerial,oldPosition={x:state.x,y:state.y},oldAwards=state.achievements.length;if(state.combat)fight(state,label);else interact(state,target,label);feedback=state.log.slice(-(state.logSerial-oldSerial||0)).filter((_,i)=>state.logSerial!==oldSerial).join(' ');if(state.x!==oldPosition.x||state.y!==oldPosition.y){motion=null;visual={x:state.x,y:state.y};}if(wasCombat&&!state.combat){actionOpen=false;toast(feedback);}else actionOpen=true;changed();announce(oldAwards);setTimeout(()=>{busy=false;render();},350);}
