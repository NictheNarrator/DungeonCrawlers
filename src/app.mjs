...
let state=fresh(),wasRegistered=true,target=null,busy=false,lastKey=0,actionOpen=false,toastTimer,bannerTimer,controls=null,feedback='',npcTab='social',camera={x:0,y:0,scale:1},visual={x:3,y:5},motion=null,frame=0;
...
function action(label,confirmed=false){if(busy||anyDialog())return;if(label==='Kill'&&!confirmed){modal('Kill '+NPCS[target].name+'?',state.npcs[target].condition==='unconscious'?'They are unconscious and cannot fight back. This kills them permanently in this playthrough.':NPCS[target].name+' will fight back. This starts lethal combat.',[['Cancel',()=>{}],['Kill',()=>action(label,true)]]);return;}stopHold();busy=true;const wasRegistered=state.registered,wasCombat=!!state.combat,oldSerial=state.logSerial,oldPosition={x:state.x,y:state.y},oldAwards=state.achievements.length;if(state.combat)fight(state,label);else interact(state,target,label);feedback=state.log.slice(-(state.logSerial-oldSerial||0)).filter((_,i)=>state.logSerial!==oldSerial).join(' ');if(state.x!==oldPosition.x||state.y!==oldPosition.y){motion=null;visual={x:state.x,y:state.y};}if(wasCombat&&!state.combat){actionOpen=false;toast(feedback);}else actionOpen=true;changed();announce(oldAwards);if(!wasRegistered&&state.registered)registerFlow();if(state.collapsed&&!wasCombat)collapseFlow();setTimeout(()=>{busy=false;render();},350);}
...
function render(){
 ...
 $('mode').textContent+=companionBadge();document.body.classList.toggle('pre-dungeon',!state.registered);
 ...
 $('relationships').replaceChildren(...[factionCard(),questCard(),companionCard(),...PEOPLE.map(id=>{const n=state.npcs[id],card=document.createElement('article');const h=document.createElement('strong');h.textContent=NPCS[id].name+' · '+n.attitude+' · '+n.condition+' · '+memberStanding(state,id);const p=document.createElement('p');p.textContent=memoryText(n)+'. Remaining possessions: '+stockText(n)+'.';card.append(h,p);return card;})]);
...
function sprite(x,y,type,condition='conscious'){const px=x*64,py=y*64;rect(px+15,py+45,36,9,'#10191888');if(condition==='dead'||condition==='unconscious'){rect(px+15,py+39,36,12,'#77695c');rect(px+42,py+33,10,10,'#ac9674');ctx.font='13px monospace';ctx.fillStyle=condition==='dead'?'#df8f80':'#b5d6e9';ctx.fillText(condition==='dead'?'×':'Zz',px+25,py+24);return;}
 if(['player','mara','tobin','vex','rat'].includes(type)){let coat=({player:'#c7dc94',mara:'#c3916b',tobin:'#dfb366',vex:'#ae9fcf',rat:'#94a387'})[type];rect(px+23,py+13,21,18,type==='rat'?'#a5aa89':'#d8b991');rect(px+20,py+29,27,21,coat);rect(px+21,py+48,9,9,'#192a2b');rect(px+37,py+48,9,9,'#192a2b');rect(px+16,py+31,6,16,coat);rect(px+47,py+31,6,16,coat);rect(px+32,py+19,4,4,'#172222');rect(px+41,py+19,4,4,'#172222');if(type==='player'){rect(px+22,py+8,23,9,'#3b5750');rect(px+16,py+13,35,5,'#3b5750');rect(px+26,py+33,5,17,'#7b905f');}if(type==='tobin'){rect(px+15,py+33,12,19,'#775232');rect(px+19,py+10,27,8,'#ae8442');}if(type==='vex'){rect(px+22,py+30,24,10,'#626886');rect(px+51,py+15,4,30,'#d7dfdf');rect(px+47,py+41,13,4,'#cbae76');}if(type==='rat'){rect(px+18,py+7,11,11,coat);rect(px+39,py+7,11,11,coat);rect(px+43,py+24,10,7,'#babc94');}return;}
